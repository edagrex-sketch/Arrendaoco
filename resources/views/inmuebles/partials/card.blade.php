<div id="inmueble-card-{{ $inmueble->id }}" class="group bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden transition-all duration-300 hover:shadow-md hover:-translate-y-1">
    {{-- Imagen --}}
    ...
    {{-- Contenido --}}
    <div class="p-6">
        <h3 class="font-bold text-lg text-[#003049] line-clamp-1 mb-1">{{ $inmueble->titulo }}</h3>
        ...
        <div class="flex items-center justify-between py-4 border-t border-slate-100 mb-6">
            <div class="flex items-center gap-4">
                ...
                <div class="flex items-center gap-1.5 text-slate-500">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" class="w-5 h-5 opacity-70" stroke="currentColor" stroke-width="2">
                    ...
                    <span class="text-base font-bold text-slate-700">{{ $inmueble->habitaciones }}</span>
                </div>
                {{-- Baños --}}
                <div class="flex items-center gap-1.5 text-slate-500">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" class="w-5 h-5 opacity-70" stroke="currentColor" stroke-width="2">
                        <path d="M9 6 6.5 3.5a1.5 1.5 0 0 0-1-.5C4.683 3 4 3.683 4 4.5V17a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-5"/><line x1="2" x2="22" y1="12" y2="12"/>
                    </svg>
                    <span class="text-base font-bold text-slate-700">{{ $inmueble->banos }}</span>
                </div>
            </div>
            @auth
                @if(Auth::id() === $inmueble->propietario_id)
                    <div class="px-2 py-0.5 rounded-full bg-red-100 text-[9px] font-black text-red-600 uppercase tracking-tighter shadow-sm border border-red-200">Tu Propiedad</div>
                @endif
            @endauth
        </div>
